...
내가 선호하는 해결책은 [i[`goto` statement]] `goto`문을 사용합니다.
...
if ((err = select(fdmax+1, &readfds, NULL, NULL, NULL)) == -1) {
    if (errno == EINTR) {
        // 어떤 시그널이 우리에게 인터럽트를 걸었다. 그러니 재시작합시다.
        goto select_restart;
    }
    // 진짜 오류는 여기서 처리하자
    perror("select");
...
물론 여기에서 `goto`를 쓸 *필요*는 없습니다. 처리를 위해 다른 구조를 쓸 수 있습니다.
그러나 저는 `goto`문이 사실 더 깔끔하다고 생각합니다. (역자 주 : 그래도
...
int recvtimeout(int s, char *buf, int len, int timeout)
{
    ...
    // 여기에 데이터가 있어야합니다. 그러니 평범한 recv()를 한다
...
생각해보세요. 각 단계는 데이터를 특정한 방법으로 바꾼다.
...
**"`PF_INET`"이 계속 등장하는데 무엇인가? `AF_INET`와 관계가 있을까요?**
...
보내면 어떻게 되겠는가? 여러분의 계정이 가진 모든 것을 삭제할 것입니다!
...
교체하세요. 저는 `CreateProcess()`를 어떻게 쓰는지는 모른다. 그것은 수억개의
...
접근하는 것을 막는 것입니다. 그러므로 그것을 허용하는 것은 보안에 헛점을 만든다.
...
어떻게 하면 내 이더넷 인터페이스를 무차별 모드로 설정할 수 있을까요?**
...
**어떤 포트가 사용 가능한 상태인지는 어떻게 알아내는가? "공식적인" 포트 번호 목록같은 것이 있을까요?**
...
오류가 발생할 것입니다. 다른 포트를 고르라. (여러분의 소프트웨어의 사용자가
