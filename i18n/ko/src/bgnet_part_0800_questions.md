...
여러분에게는 다행히도 사실상 모든 장치가 커널에 자리잡고 네트워크 카드인 척 하는
...
시그널은 블록된 시스템 콜이 `errno`를 `EINTR`로 설정하고 `-1`을 반환하게
...
[`select()`](#select)를 사용하세요! 그것은 읽어들이려는 소켓 설명자에 시간 제한
...
그러나 모든 것이 안 된다고 말하려고 이 이야기를 꺼낸 것은 아닙니다. 방화벽이
...
방화벽은 그것의 NAT 소프트웨어나 프록시 등을 써서 여러분에게 연결을 전달
해줄 수 있습니다.
...
일 때 목적지 주소가 실행 중인 장치가 아닌 패킷까지 전부 운영체제에 전달합니다.
