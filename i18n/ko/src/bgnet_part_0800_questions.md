...
[i[`route` command]] `route` 명령(대개의 리눅스 장치에서 `/sbin`에 있습니다)
...
여러분에게는 다행히도 사실상 모든 장치가 커널에 자리 잡고 네트워크 카드인 척하는
...
간단히 말하자면 네트워크 없는 단일 장치에서 코드를 실행하기 위해 코드를 변경할
필요는 없습니다. 만세!
...
링커 오류는 Sun 장치들이 자동으로 소켓 라이브러리를 링크하지 않기 때문에
...
로컬 장치에서는 루프백 장치를 쓰기 때문에 8K나 그 이상의 크기도 문제없이 다룰 수 있습니다.
...
**저는 Windows 장치를 써서 `fork()` 시스템 호출이 없고 `struct sigaction` 같은
것도 없습니다. 어떻게 해야 하나요?**
...
일 때 목적지 주소가 현재 장치가 아닌 패킷까지 전부 운영체제에 전달합니다.
...
와 같은 포트를 씁니다. 중요한 것은 *같은 장치*의 누구도 여러분이 그 포트를
