...
여러분이 [i[Goat]] "`goat`"라는 이름의 장치에 로그인했다고 가정해볼까요? 클라이언트를
하나의 창에서 실행하고 서버를 다른 창에서 실행해보세요. 아니면 서버를 백그라운드에서
실행하고("`server &`") 클라이언트를 같은 창에서 실행해도 좋습니다. 루프백 장치는 여러분이
...
이 완료되었다는 의미입니다. 이 시점에서 `getsockopt()`을 `SO_ERROR`옵션과 함께
...
`select()`를 다시 호출해보세요. `select()`가 20번 정도 시간초과를 일으킨다면
...
닫혔을 때 `0`을 돌려준다는 점을 떠올려보세요. 그러니 그 값은 쓸 수가 없고,
...
처리한다고 가정해볼까요? (즉 클라이언트가 연결을 닫지 않고 후속 시스템 콜을 보내는
일은 없다는 의미입니다.)
...
와 같은 포트를 쓴다고 합니다. 중요한 것은 *같은 장치*의 누구도 여러분이 그 포트를
