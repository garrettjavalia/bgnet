...
## 옮긴이의 말: 2023년 번역, 원문 3.1.5 기반 한국어판 {#translators-note-2023}
...
이 문서의 번역본은 1998년에 최초로 한국어 버전이 작성된 이래로 한국인이 Socket 프로그래밍에 대해서 참고할 수 있는 문서 중 리눅스 맨페이지를 제외하면
...
## 옮긴이의 말: 2026년 개정, 원문 3.3.2 기반 한국어판 {#translators-note-2026}

원문 3.1.5 버전을 번역한 뒤 몇 해가 지나, 개정판을 다시 작업하게 되었습니다.

3.1.5 한국어판에 있던 수많은 오탈자와 잘못된 번역을 개선했으며, 기존에는
분량상의 이유로 번역하지 못했던 맨페이지 부분까지 번역했습니다. 기존 번역물은
문서 전체에 걸쳐 Pretendard 폰트를 사용했습니다. 이번에도 기본 폰트는 동일하지만
코드 스니펫에는 Naver D2 Coding 폰트를 사용해서 한글 주석이 조금 더 코드와
잘 어울리게 했습니다.

문서 전채를 개정하는 대규모 작업은 AI 덕분에 가능했습니다. 이전 3.1.5 한국어판은 인터넷
검색과 수작업으로 진행했으나, 이번 작업은 거의 모든 부분을 AI의 도움으로
