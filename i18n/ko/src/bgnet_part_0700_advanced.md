...
이만하면 충분히 이야기했습니다. 이제 이것을 어떻게 쓰는지 살펴볼까요?
...
한다는 의미가 아닌가요? 맞아요! 그리고 연결이 닫힐 때마다 `master`집합에서 제거해야
하지 않나요? 맞아요, 그렇게 해야 합니다.
...
세 번째 방법을 선호합니다. 다만 그 전에 다른 두 가지 방법의 장단점에 대해서 조금 더 알아볼까요?
...
좋습니다. 예를 들자면 `SOCK_STREAM`을 사용하는 다중 사용자 대화 프로그램이 있다고 가정해볼까요?
...
그것이 들어오는 대로 클라이언트에게 `send()`한다고 가정해볼까요? 여러분의 송출 자료 스트림은
아래와 같을 것입니다.
...
이라고 부른다고 합니다) 알고있습니다. 지금 이해할 필요는 없지만 우리는 클라이언트와
서버가 어떻게 통신하는지를 정의하는 *프로토콜*을 정의하려고 하고있습니다.

지금은 사용자의 이름이 `'\0'`으로 패드된 고정된 8개의 문자라고 가정해볼까요?
데이터는 최대 128개 문자로 구성되는 가변길이 형태라고 해 보죠. 이 상황에서
쓸 수 있는 예제 패킷 구조를 살펴볼까요?
...
특별한 이유는 없고, 충분히 길 것이라 생각했습니다. 그러나 아마도 8바이트는 여러분의
...
대해서 이야기했습니다. 그러나 적절한 권한이 있다면 _한 번에_ 여러 호스트에게
자료를 보낼 수 있습니다!
...
여기서 잠시 접어두죠. 우리는 32비트의 현재에 갇혀있습니다.
...
자, 고양이 가죽을 벗기는 일에도 여러 방법이 있을 수 있으니 잠깐만 생각해볼까요?
...
응용프로그램의 *유일한 차이*입니다. 그러나 오래된 `talker` 응용프로그램에
`SO_BROADCAST` 소켓 옵션을 설정하는 부분을 하나 추가해보겠습니다. 이 프로그램을
...
`broadcaster`에 브로드캐스트 주소를 넣고 다시 실행해볼까요? `sendto()`를 한 번만
