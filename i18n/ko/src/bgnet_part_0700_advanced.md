...
피할 수 있습니다. (역자 주 : poll은 투표, 설문, 그러한 부류의 조사라는 뜻이다.) 간단히
...
설정합니다. 그리고 각각의 `events`필드는 우리가 관심있는 이벤트로 설정하는 것이다.
...
밀리초 단위의 제한시간도 같이 넘겨야 합니다.(영원히 기다리려면 음수를 지정하면 된다.)
...
`fd_set`형이다. 이 자료형에 대해서 아래의 매크로들을 쓸 수 있습니다.
...
방법이라고 생각할 것이다. 그리고 맞다. 맞을 _수도_ 있습니다. 일부 유닉스에서는
...
이 예제에서 `s`는 여러분이 데이터를 보내고 싶은 소켓이고 `buf`는 자료를 담은 버퍼이다.
...
의미합니다. ("이건 되고", "이건 안되고"를 듣는 것이 지겨운가? 미안하다.
...
**      - |                               s
**
**  (문자열은 저장된 길이에 근거해서 추출된다. 그러나 `s`의 앞에 최대 길이를 앞에 지정할 수 있습니다.)
*/
...
다른 값으로 설정할 수 있습니다. 선택은 여러분의 몫이다.
...
총 갯수를 알고있습니다. 갯수가 패킷의 앞쪽에 붙어있기 때문이다. 우리는 또한 패킷의
