...
기다리고 있다는 것을 눈치채셨을 것입니다. 내부적으로는 `recvfrom()`이 호출되고,
데이터가 없으므로 `recvfrom()`는 데이터가 도착할 때까지 "블록"된 상태인 것입니다.
(즉 그대로 잠들어 있게 됩니다.)
...
해당하는 종류의 이벤트가 발생(예를 들어 "소켓에서 데이터를 읽을 수 있다!" 같은
이벤트)하거나 사용자가 지정한 제한 시간이 지날 때까지 `poll()` 호출은 블록됩니다.
...
비슷하게 클라이언트 연결이 읽을 준비가 되고 `recv()`가 `0`을 반환한다면 클라이언트가
...
(이 코드는 위의 `pack754()` 함수를 참조합니다. `packi*()` 함수는 정수가 아닌
`char` 배열에 수를 담는다는 점을 제외하면 `htons()` 계열 함수와 유사하게
동작합니다.)
