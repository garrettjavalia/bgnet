...
많은 함수가 블로킹됩니다. `accept()`와 모든 `recv()` 함수도 블로킹됩니다.
이 함수들이 블로킹될 수 있도록 허용되어 있기 때문입니다.
`socket()`으로 소켓 설명자를 처음 만들 때 커널이 이 소켓을 블로킹
소켓으로 설정합니다. [i[Non-blocking sockets]] 소켓이 블로킹되지 않게 하려면
...
같은 이벤트)하거나 사용자가 지정한 제한 시간이 지날 때까지 `poll()` 호출을 블로킹할
것입니다.
...
쓰면 될 뿐입니다. 하지만 정말로 그럴까요? `accept()` 호출이 블로킹되었다면
...
가 블로킹될 것임을 의미합니다. 이런! 아무튼 해결책은 읽을 소켓에 [i[`O_NONBLOCK` macro]]
