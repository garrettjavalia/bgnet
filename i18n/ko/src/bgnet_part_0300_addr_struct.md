...
이 구조체는 나중에 사용할 소켓 주소 구조체를 준비하는 데 쓰입니다. 또한 호스트 이름 찾기나 서비스 이름 찾기
에도 사용됩니다. 나중에 실제 사용 예시를 보시면 이해가 되겠지만, 지금은 연결을 만들 때 가장 먼저
...
이 구조체에 내용을 조금 채워 넣은 후에 [i[`getaddrinfo()` function]] `getaddrinfo()`
...
입니다. 이것은 숫자와 점 표기법으로 적힌 IP 주소를 여러분이 `AF_INET` 또는 `AF_INET6`
...
어떻게 해야 할까요?) 이 경우 여러분은 [i[`inet_ntop()` function]] `inet_ntop()`
...
하지 않습니다. 그 작업을 위해서는 다음에 보실 `getaddrinfo()`을 써야 합니다.
